# PROJECT BRICK WARFARE

God Mode에서 생산망과 부대를 지휘하다가 결정적인 순간 아군 유닛을 직접 조종하는 3D 블록 전술 게임입니다. 기본 모드는 7개 거점을 두고 싸우는 12분 정복전이며, 3분 AI Challenge와 무제한 Sandbox도 제공합니다.

- **플레이:** https://wlsalswo14.github.io/WARGAMES/
- **AI Challenge:** https://wlsalswo14.github.io/WARGAMES/?mode=challenge
- **1:1:1:** https://wlsalswo14.github.io/WARGAMES/?mode=challenge&factions=3
- **Sandbox:** https://wlsalswo14.github.io/WARGAMES/?mode=sandbox
- **소스:** https://github.com/wlsalswo14/WARGAMES

## 게임 한눈에 보기

### Conquest

- 12분 동안 이어지는 메인 게임 모드
- 도시·참호·고지대가 연결된 중립 거점 7개
- 거점 수입과 생산시설 수입을 이용하는 보급 경제
- 보병 막사·기갑 공장·전술 비행장 건설
- 드래그 다중 선택과 횡대·쐐기·종대 대형 명령
- 정밀 포격·대전차 공습·긴급 증원·야전 수리 지휘 능력
- 적 AI의 거점 공략, 생산시설 확장, 생산망 습격, 본부 공세
- 전 거점 30초 장악, 적 본부 파괴, 적 전멸 또는 시간 종료로 승패 판정
- 전투 결과 XP, 계급, 다음 경기 시작 보급 보너스 저장

### AI Challenge

- `1:1` 또는 `1:1:1`
- 제한 시간 3분
- 중립 거점 `A / B / C`
- 진영별 시작 병력 10기
- 보병·장군·전차·드론·헬기·전투기 혼성 편성
- 시가전·능선전·참호전 중 무작위 전장
- 100 지휘 점수 선취
- 아군 유닛 즉시·무제한 직접 조종
- 생존 본진·점령 거점·직접 건설한 생산기지에서 병력 증원
- 플레이어 명령 성향에 대응하는 적응형 적 지휘관

거점 점령은 8점, 적 격파는 2점, 적 장군 제거는 12점, 적 지휘관 기만은 5점을 제공합니다. 소유 거점은 매초 점수를 생산하며, 동시에 보유한 거점이 많을수록 점수 생산량이 빠르게 증가합니다.

### Sandbox

- 3개 진영과 자율 외교
- 무한 유닛·건물·장벽·나무 배치
- 산·참호 실시간 지형 제작
- 실행할 때마다 달라지는 절차 생성 전장
- 모든 진영 유닛 직접 조종
- 전투기·헬기 포함 전체 병과

## 핵심 플레이

1. 정찰 유닛과 기동 부대를 나눠 초기 거점을 확보합니다.
2. `Tab`으로 전술 커서를 열고 드래그해 분대를 편성합니다.
3. 점령 거점에 막사·기갑 공장·비행장을 지어 생산망을 확장합니다.
4. `1~4` 지휘 능력으로 교착된 전선을 돌파하거나 아군을 복구합니다.
5. `Enter` 또는 유닛 더블클릭으로 결정적인 유닛을 직접 조종합니다.
6. 전 거점을 완전히 장악하거나 적 지휘 본부와 병력을 파괴합니다.

## 조작

### God Mode

| 입력 | 기능 |
| --- | --- |
| 마우스 | 이동 방향과 동일한 자유 시점 |
| `W / S` | 전진 / 후진 |
| `A / D` | 오른쪽 / 왼쪽 이동 |
| `Space / Shift` | 상승 / 하강 |
| 마우스 휠 | 관찰자 이동속도 |
| 좌클릭 | 유닛 선택 |
| 우클릭 | 이동 또는 공격 명령 |
| `Tab` | 포인터 잠금 / 전술 커서 전환 |
| 드래그 | 여러 아군 유닛 선택 |
| `R` | 횡대 / 쐐기 / 종대 대형 변경 |
| `1 / 2 / 3 / 4` | 포격 / 공습 / 긴급 증원 / 야전 수리 |
| `1 / 2 / 3` | Challenge에서 A / B / C 거점 명령 |
| 생산 도크 | 유닛 생산과 전용 생산시설 건설 |
| `Enter` / 더블클릭 | 선택 유닛 직접 조종 |

### 직접 조종

| 입력 | 기능 |
| --- | --- |
| 마우스 | 이동 방향과 동일한 조준과 시점 |
| `W / S` | 전진 / 후진 |
| `A / D` | 오른쪽 / 왼쪽 이동 |
| `Space / Shift` | 헬기·드론 상승 / 하강, 전투기 기수 올림 / 내림 |
| 좌클릭 / 누르고 유지 | 일반공격 연사 |
| 우클릭 / 누르고 유지 | 특수공격, 드론 자폭 |
| `V` | 1인칭 / 3인칭 |
| `G` / `Esc` | God Mode 복귀 |

## 구현 특징

- 전투기는 `Space`를 약 8초간 눌러 360° 후방 루프를 완주할 수 있습니다. 기본 3인칭은 지평선을 유지해 위치를 보여주고, `V`로 전환하는 1인칭은 기수 방향을 따라갑니다.
- 전투기 빙의 3인칭은 카메라를 기체 위·측면으로 이동하고 선택 원과 진영 표식을 숨겨 십자선을 가리지 않습니다.

- **Three-point frontline:** 시가전·능선전·참호전 중 하나에서 세 거점을 두고 빠르게 이동하는 전선
- **Seven-front conquest:** 7개 거점, 본부, 전방 생산시설을 연결한 12분 메인 전역
- **Base economy:** 점령 거점과 생산시설이 보급 수입·병력 상한·생산 병과를 함께 결정
- **Squad command:** 드래그 다중 선택과 횡대·쐐기·종대 대형 이동
- **Commander assets:** 포격·공습·증원·수리의 자원 비용 및 재사용 대기시간 관리
- **Strategic destruction:** 생산시설이 파괴되면 해당 병과 생산·수입·병력 상한이 실제로 감소
- **Subsystem damage:** 관통 위치에 따라 차량 기동계 또는 화기계가 손상되고 야전 수리로 복구
- **Command progression:** 전투 성과를 XP와 계급으로 저장하고 다음 정복전의 시작 보급에 반영
- **Adaptive Director:** 플레이어가 반복해서 명령한 거점을 가중치로 기억하고 적 예비대를 대응 배치
- **Unit AI:** 거점 우선 진격, 이동 중에도 보이는 적 즉시 사격, 구조물 우회, 정지 상태 탈출
- **Command general:** 진영별 장군 1명은 높은 체력·장갑·점령력과 지휘 유탄을 보유하며 제거 시 12점 제공
- **Forward production:** 점령 거점당 생산기지 1개를 직접 건설해 진영 병력 상한을 최대 2기 확장
- **Active patrol:** 점령 목표가 사라져도 아군 거점 주변을 순찰하고, 막히면 좌우 탈출 경로를 번갈아 재계산
- **Dual weapons:** 보병 외 유닛은 빠른 일반공격과 느리지만 강력한 특수공격 사용
- **Drone strike:** 드론은 사격과 충돌 자폭을 모두 사용
- **Safe unit contact:** 일반 유닛끼리 충돌해도 파괴되지 않으며 자폭드론만 충돌 폭발 가능
- **Brick destruction:** 유닛·나무·구조물이 실제로 파괴될 때만 블록 파편 생성
- **Structure collapse:** 하부 지지 블록이 무너지면 상부 블록 연쇄 붕괴
- **Straight projectiles:** 모든 탄환과 폭탄은 조준 방향으로 직선 비행
- **Ground splash:** 광역 특수탄은 유닛을 직접 맞히지 않아도 지면 충돌점 주변 유닛과 구조물에 폭발 피해 적용
- **Territory feedback:** 점령한 거점 주변 지형이 해당 진영 색으로 변화
- **Procedural audio:** 음원 파일 없이 Web Audio API로 사격·폭발·점령·음악 합성

게임 내부 적응형 지휘관은 브라우저에서 동작하는 경량 규칙·가중치 시스템입니다. 외부 LLM이나 유료 API가 필요하지 않습니다.

## 로컬 실행

Node.js 24 이상을 권장합니다.

```bash
npm install
npm run dev
```

프로덕션 검증:

```bash
npm run check
npm run build
npm run preview
```

## 기술 구성

- TypeScript
- Three.js
- Vite
- WebGL / Web Audio API
- GitHub Actions / GitHub Pages

외부 이미지, 3D 모델, 음원, 폰트 파일을 포함하지 않습니다. 유닛·구조물·지형·식생·효과·사운드는 모두 코드에서 생성합니다.

## 프로젝트 구조

```text
src/
├─ game/
│  ├─ battlefield/    # 전장 배치와 테마
│  ├─ entities/       # 유닛, 발사체, 거점, 파괴 구조물
│  ├─ input/          # 키보드·마우스·포인터 잠금
│  ├─ modes/          # Conquest / Challenge / Sandbox 규칙
│  ├─ systems/        # AI, 전투, 경제, 지휘 능력, 성장, 외교
│  ├─ ui/             # HUD와 결과 화면
│  ├─ world/          # 절차 생성 지형
│  └─ BrickWarfare.ts # 메인 조율 계층
└─ main.ts
```

## 성능 설계

- Challenge 실시간 그림자 비활성화
- Challenge AI 15 Hz, HUD 5 Hz, 거점 판정 10 Hz
- Conquest 내부 해상도 상한 1.1배, 발사체 최대 96개
- 소프트웨어 렌더러에서는 실시간 그림자 비활성화
- 구조물 블록을 재질별 `InstancedMesh`로 렌더링
- 진영별 기본 병력 10기, 생산기지 가동 시 최대 12기
- Challenge 발사체 최대 72개
- 구조물별 잔해 최대 48개, 전역 장식 파편 최대 104개
- 파괴된 유닛과 GPU 리소스 자동 정리
- 소프트웨어 렌더러 감지 시 내부 해상도 자동 축소

브라우저 하드웨어 가속을 켜면 GPU로 WebGL을 렌더링합니다. 하드웨어 가속이 꺼진 환경에서는 경고와 함께 저해상도 안전 모드를 적용합니다.

GitHub Pages는 정적 파일만 전달합니다. 전투 AI, 생산, 점령과 물리는 플레이어 브라우저에서 실행되므로 생산기지를 건설해도 GitHub 서버에 시뮬레이션 부하가 생기지 않습니다.

## 제출물

- [게임 소개 및 설명 PDF](submission/PROJECT_BRICK_WARFARE_GAME_GUIDE.pdf)
- [AI 활용 기술 문서 PDF](submission/PROJECT_BRICK_WARFARE_AI_TECHNICAL_DOCUMENT.pdf)
- [30.52초 실제 플레이 영상](submission/PROJECT_BRICK_WARFARE_GAMEPLAY_31S.webm)
- [게임 설명 원문](docs/GAME_GUIDE.md)
- [AI 기술 문서 원문](docs/AI_TECHNICAL_DOCUMENT.md)
- [코드 구조와 변경 위치](docs/ARCHITECTURE.md)
- [제출 체크리스트](docs/SUBMISSION_CHECKLIST.md)
- [YouTube 업로드 정보](submission/YOUTUBE_UPLOAD.md)

## 오픈소스 및 권리

- Three.js — MIT License
- Vite — MIT License
- TypeScript — Apache License 2.0

특정 완구 회사의 상표·캐릭터·모델을 사용하지 않는 독립적인 블록 기반 게임입니다.
